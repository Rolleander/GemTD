...
func _get_recipe_matches(gems: Array, recipe: Array[BasicGemId]):
	var recipe_material = [] as Array[Gem]
	for r in recipe:
		var matching: Gem = null
		for gem in gems:
			var matches_id = gem.quality == r.quality && gem.type == r.type
			var available = gem.special_combination == null && !recipe_material.has(gem)
			if matches_id && available:
				matching = gem
				break
		if matching == null:
			return [] as Array[Gem]
		recipe_material.append(matching)

	# Once the recipe is proven possible, let any matching gem be chosen as its
	# target. GemCombine will select the remaining materials by EXP.
	var target_gems = [] as Array[Gem]
	for gem in gems:
		if gem.special_combination != null:
			continue
		for r in recipe:
			if gem.quality == r.quality && gem.type == r.type:
